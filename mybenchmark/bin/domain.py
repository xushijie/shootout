...
class Record(object):
   ...
   def __str__(self):
      return '%d,%d,%.3f,%d,%d,%s,%.3f, %s' % (
         self.arg, self.gz, self.userSysTime, self.maxMem, self.status, self.cpuLoad, self.elapsed, str(self.footprint))
...
class DataRecord(object):
   ...
   def __str__(self):
      return '%s,  %s,  %.3f,   %d,    %.3f, %s' % (
         self.argString, self.timestamp, self.maxMem, self.status, self.elapsed, str(self.footprint))
