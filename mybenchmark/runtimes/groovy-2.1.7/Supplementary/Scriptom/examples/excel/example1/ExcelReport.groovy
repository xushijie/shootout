...
final reportFile = new File('./report.xls').canonicalFile
...
helper.create(new File("./reporttemplate.xls"), reportFile)
{workbook ->
  ...
  assert 'Excel.Sheet' == Excel.progIds.Worksheet
  assert 'Excel.Sheet' == Excel.progIds[Excel.coClasses.keySet().find{name->worksheet.supportsInterface(Excel.coClasses[name])}]
...
helper.process(new File("./report.xls"))
{workbook ->
